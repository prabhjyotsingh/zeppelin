...
abstract public class AbstractZeppelinIT {
  ...
  protected void createNewNote() {
    if (driver.findElements(By.xpath("//div[contains(@class, \"col-md-4\")]"
        + "/div/h5/a[contains(.,'Create new note')]")).size() == 0) {
      driver.findElement(By.className("navbar-title")).click();
    }
  ...
  protected static void setImplicitlyWaitDefault() {
    driver.manage().timeouts().implicitlyWait(AbstractZeppelinIT.MAX_IMPLICIT_WAIT,
        TimeUnit.SECONDS);
  }

  protected static void setImplicitlyWaitMinimum() {
    driver.manage().timeouts().implicitlyWait(AbstractZeppelinIT.MIN_IMPLICIT_WAIT,
        TimeUnit.SECONDS);
  }
